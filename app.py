...
def read_schema():
    ...
    # Try to align to expected column names
    columns = {c.lower().strip(): c for c in df.columns}
    # Expected logical names
    # Attempt to map to: Variable, Description, Field_Type, Suggested_Weight
    mapping = {}
    for logical, candidates in {
        "Variable": ["variable","field","name","Field","Variable"],
        "Description": ["description","desc","purpose","Description"],
        "Field_Type": ["field type","type","Field Type","Field_Type"],
        "Suggested_Weight": ["suggested weight (%)","weight","weight %","Suggested Weight (%)","Suggested_Weight"]
    }.items():
        for cand in candidates:
            lc = cand.lower()
            if lc in columns:
                mapping[logical] = columns[lc]
                break
    # Ensure defaults
    if "Variable" not in mapping:
        mapping["Variable"] = df.columns[0]
    if "Description" not in mapping and len(df.columns)>1:
        mapping["Description"] = df.columns[1]
    if "Field_Type" not in mapping:
        df["Field_Type"] = "Text"; mapping["Field_Type"] = "Field_Type"
    if "Suggested_Weight" not in mapping:
        df["Suggested_Weight"] = 1; mapping["Suggested_Weight"] = "Suggested_Weight"

    use = df[[mapping["Variable"], mapping["Description"], mapping["Field_Type"], mapping["Suggested_Weight"]]].copy()
    use.columns = ["Variable","Description","Field_Type","Suggested_Weight"]
    use = use.fillna("")
    return use
...
def admin_only(f):
    ...
    def w(*a,**k):
        if current_user.role!='admin':
            flash('Admin access required'); return redirect(url_for('home'))
        return f(*a,**k)
    return w
